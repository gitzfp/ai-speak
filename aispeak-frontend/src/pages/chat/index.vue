  ...
  <view class="chat-box">
   ...
   <!-- 底部操作栏 -->
    <view v-if="!session.completed" class="chat-bottom-container">
      ...
      <view v-if="!inputTypeVoice" class="input-bottom-container" :style="'bottom:' + inputBottom + 'px;'">
        <view @tap="handleSwitchInputType" class="voice-icon-box">
          <image class="voice-icon" src="https://api.zfpai.top/static/icon_voice_fixed.png"></image>
        </view>
        ...
        <view @tap="handleSendText" class="send-icon-box" :class="{ active: inputHasText }">
          <image class="send-icon" src="https://api.zfpai.top/static/icon_send.png"> </image>
        </view>
      </view>
        ...
        <view class="speech-box">
          <Speech :session-id="session.id">
            <template v-slot:leftMenu>
              <image @tap="handleSwitchInputType" class="keybord-icon" src="https://api.zfpai.top/static/icon_keybord.png"></image>
            </template>
            <template v-slot:rightMenu>
              <image @tap="handleSwitchMenu" class="input-type-switch-btn" src="https://api.zfpai.top/static/icon_settings.png"></image>
            </template>
          </Speech>
        </view>
      </view>
    ...
    <view class="finish-buttons" v-if="session.completed" >
      <button class="practice-again" @tap="handlePracticeAgain">再次练习</button>
      <button class="feedback-button" @tap="completeTopic">查看评分</button>
    </view>

    <!-- 悬浮按钮 -->
    <view class="floating-button" @tap="handleShowModal">
      <image src="https://api.zfpai.top/static/img/note.png" mode="aspectFit" />
    </view>
      ...
      <view class="modal-content">
        ...
        <view class="modal-body">
          ...
          <view class="section">
            ...
            <text class="section-content">你需要在4轮之内完成以下任务，每完成一个，会获取⭐奖励（点击右下角提示随时查看任务）</text>
          </view>
          <view class="section">
            <text class="section-title">请完成以下目标：</text>
            <view class="section-content">
              <view v-for="(target, index) in lessonData.task_target" :key="index" class="target-item">
                ...
                <view class="target-text">
                  ...
                  <view class="audio-icon" @tap="playAudio(target.info_en)">
                    <image src="https://api.zfpai.top/static/icon_audio.png" mode="aspectFit"></image>
                  </view>
                </view>
              </view>
            </view>
  ...
  chatRequest
    ...
    .then((data) => {
      ...
      if (data.achieved_targets) {
          if (data.achieved_targets?.length > 0 && message === data.achieved_targets[data.achieved_targets.length - 1].user_say) {
