...
class StudyWordProgress(Base):
    ...
    last_study_time = Column(Date, comment="最后一次学习时间")  # 修改为 Date 类型
    next_review_time = Column(Date, comment="下次复习时间（未掌握时有效）")  # 修改为 Date 类型
    is_mastered = Column(Integer, default=0, comment="是否已掌握（0=未掌握，1=已掌握）")
    study_date = Column(Date, comment="学习日期")
    type = Column(Integer, default=1, comment="类型字段，默认值为1")
    create_time = Column(DateTime, default=datetime.datetime.now, comment="创建时间")  # 新增字段
